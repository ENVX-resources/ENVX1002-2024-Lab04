...
```{r}
1-pnorm(180,163.9,6.6) ## actual p
...
ggplot(data.frame(x = c(4.16-4*0.3, 4.16+4*0.3)), aes(x = x)) +
  stat_function(fun = dnorm, args = list(mean = 4.16, sd=0.30)) +
  stat_function(fun = dnorm, args = list(mean = 4.16, sd=0.30)
    , xlim = c(4.16-4*0.3, 4), geom = "area", fill = "lightblue") +
  xlab("x") +
  ylab(expression(N(4.16,0.30^2)~pdf))
```
...
Step 1: Using gplot, draw a sketch of the Normal curve with the probability identified. You may need to draw a section of the right tail as the probability is small!

Step 2: Calculate the probability in R or write out the probability, standardise the variable, and look up the Normal Table.
